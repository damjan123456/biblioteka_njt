...
public interface StavkaPozajmiceRepo extends JpaRepository<StavkaPozajmice, Integer> {
    ...
    // Postoji li preklapanje za BILO koju od zadatih knjiga u intervalu?
    @Query("""
    ...
    // Postoji li preklapanje za konkretnu knjigu u intervalu?
    @Query("""
      select (count(s) > 0) from StavkaPozajmice s
      join s.pozajmica p
    ...
    List<StavkaPozajmice> findByPozajmica_Id(Integer pozajmicaId);
}
